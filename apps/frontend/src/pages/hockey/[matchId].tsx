...
export default function HockeyMatchStreamsPage() {
  ...
  const [currentStream, setCurrentStream] = useState<HockeyStreamDto | null>(null);
  ...
  useEffect(() => {
    ...
    getHockeyStreams(matchId).then((allStreams) => {
      ...
      if (!streamNo && selected) {
        navigate(`/hockey/${matchId}/stream/${selected.streamNo}`, { replace: true });
      }
      setLoading(false);
    });
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-4 sm:py-6">
      ...
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-6">
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-[#0f0f13]">
                <div className="flex flex-col items-center gap-4 text-center px-6">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-sm">This stream can't be embedded. Try opening it directly or switch to another source.</p>
                  </div>
                  <a
                    href={currentStream.embedUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 px-5 py-2.5 bg-indigo-500 hover:bg-indigo-400 text-white text-sm font-bold rounded-xl transition-colors"
                  >
                  ...
                  {otherStreams.length > 0 && (
                    <p className="text-xs text-slate-600">or try another stream below</p>
                  )}
                </div>
              </div>
            ) : (
        ...
        {/* Right sidebar — horizontal scroll cards on mobile, vertical panel on desktop */}
